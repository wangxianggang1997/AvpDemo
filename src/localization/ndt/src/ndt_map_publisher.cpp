...
{
  try {
    //加载yaml文件->autonomoustuff_parking_lot_lgsvl.yaml
    ...
    if (map_info["map_config"]) {
        ...
        map_info["map_config"]["elevation"])
      {
        ...
        //yaml文件中的roll pitch yaw 赋值给geo_pose
...
void read_from_pcd(const std::string & file_name, sensor_msgs::msg::PointCloud2 * msg)
{
  ...
  if (pcl::io::loadPCDFile(file_name, pcl_cloud) == -1) {  // 加载pcd文件
  ...
  // 将PCL下PointCloud2转换成sensor_msgs下的PointCloud2格式
  ...
  //确认至少有x,y,z属性，检查是否有强度intensity信息
  ...
  for (size_t i = 0; i < num_points; ++i) {
    ...
    // If intensity exists, copy it into the new cloud, otherwise, set it to 0.04
    //如果强度信息存在，将它复制到新点云中，否则设置为0.04
