...
{
  // Verify frame_id 验证frame_id，如果帧数据的frame_id不是m_input_frame_id则抛出异常
  ...
  sensor_msgs::PointCloud2ConstIterator<float32_t> x_it(msg, "x");//获取位置x坐标
  sensor_msgs::PointCloud2ConstIterator<float32_t> y_it(msg, "y");//获取位置y坐标
  sensor_msgs::PointCloud2ConstIterator<float32_t> z_it(msg, "z");//获取位置z坐标

  auto && intensity_it = autoware::common::lidar_utils::IntensityIteratorWrapper(msg);//激光强度
