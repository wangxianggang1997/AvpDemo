...
: VehicleInterfaceNode{"ssc_interface", options}
{
  set_interface(
    std::make_unique<SscInterface>(
      *this,
      declare_parameter("ssc.front_axle_to_cog").get<float32_t>(), //前轴到变速箱距离
      declare_parameter("ssc.rear_axle_to_cog").get<float32_t>(),  //后轴到变速箱距离
      get_state_machine().get_config().accel_limits().max(),       //最大加速度
      get_state_machine().get_config().accel_limits().min(),       //最小加速度
      declare_parameter("ssc.max_yaw_rate").get<float32_t>()       //最大航向角
  ));
}
